    ...
    <div class="layout-principal">
      <div class="columnas-wrap">
        <section class="productos-seccion">
          <h1 class="titulo-pizarra">Menu Online</h1>
          <p class="subtitulo-pizarra">Elegi tu bebida y comida artesanal.</p>
...
const sincronizarMesaInicial = () => {
  const mesaGuardada =
    localStorage.getItem("mesa_id") || authStore.user?.mesa_id;

  if (mesaGuardada) {
    carritoStore.setMesa(Number(mesaGuardada));
  }
};
...
const enviarPedidoFinal = async () => {
  ...
  if (tieneItemsSinStock) {
    mensajeError.value = "Producto sin Stok.";
  ...
  let idMesaDestino = carritoStore.mesaId;
  if (!idMesaDestino) {
    idMesaDestino = localStorage.getItem("mesa_id");
    if (idMesaDestino) {
      carritoStore.setMesa(Number(idMesaDestino));
    }
  }

  if (!idMesaDestino) {
    mensajeError.value = "Mesa no existe o Ocupada.";
    mostrarModalError.value = true;
    return;
  }

  const payload = {
    mesa: Number(idMesaDestino),
  ...
  try {
    ...
    if (error.response && error.response.data && error.response.data.error) {
      ...
      mensajeError.value =
        "Error: Algunos productos elegidos superan el stock disponible .";
    } else {
      mensajeError.value =
        "Ocurrio un problema al enviar el pedido a la cocina.";
    }
    mostrarModalError.value = true;
    await fetchProductos();
  }
